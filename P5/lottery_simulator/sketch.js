...
function setup() {
  ...
  p51 = createP("frequency:");
  p51.id("frequency");
  p51.style("font-size", text_size);
  p51.style("color", black);
  p51.position(20, 260);
...
function buy_one_lottery() {
  ...
  document.getElementById("my_selection_red").innerHTML =
    JSON.stringify(my_red_numbers);
  document.getElementById("my_selection_blue").innerHTML =
    JSON.stringify(my_blue_numbers);
  compare_result();
  document.getElementById("red_balls_matched").innerHTML =
    JSON.stringify(red_matches);
  document.getElementById("blue_balls_matched").innerHTML =
    JSON.stringify(blue_matches);
  ...
  document.getElementById("net_gain").innerHTML =
    "net gain: " + (spendings + earnings);

  let total_lottery = (-1 * spendings) / 2;
  document.getElementById("frequency").innerHTML = `frequency: <br />
    total:
    ${total_lottery}
    <br/>
    1st prize:
...
function get_lottery_result() {
  ...
  document.getElementById("red_balls").innerHTML =
    "red balls: " + JSON.stringify(this_red_result);
  document.getElementById("blue_balls").innerHTML =
    "blue balls: " + JSON.stringify(this_blue_result);
}
...
function compare_result() {
  ...
  if (red_matches.length === 6 && blue_matches.length === 1) {
    ...
    (red_matches.length === 4 && blue_matches.length === 1) ||
    (red_matches.length === 5 && blue_matches.length === 0)
  ) {
    earnings += 200;
    win_amount = 200;
    win = true;
    ...
    (red_matches.length === 3 && blue_matches.length === 1) ||
    (red_matches.length === 4 && blue_matches.length === 0)
  ) {
    earnings += 10;
    win_amount = 10;
    win = true;
